...
This module orchestrates the flow of data from input sources through detection and music generation pipelines. 
It manages queues, worker threads, and state synchronization for real-time processing.
"""
...
class Processor:
    """
    Orchestrates turning queued input (video frames or generic sequential events) into music.
    """

    def __init__(self, socketio_instance=None):
        """Initialize the processor: pick a frame pipeline, start worker threads."""

        self.socketio = socketio_instance   # Store socketio instance for broadcasting
        self.frame_counter = 0

        self.frame_queue = Queue(maxsize=10)
        self.event_queue = Queue(maxsize=200)

        self.current_frame = None
        self.current_display = None         # last FramePipeline display_payload
        ...
        self.processing_interval = 2        # Process every Nth frame for performance (1 = every frame)
        ...
        # Display Pipeline chooser to user in terminal
        print("CHOOSE A PROCESSING PIPELINE:")
        for key, name in available_pipelines.items():
            print(f"  {key}. {name}")
        choice = input("Enter your choice (1-3): ")
        try:
            choice = int(choice)
            if choice in available_pipelines:
                pipeline_name = available_pipelines[choice]
                logger.info("🔄 Initializing %s...", pipeline_name)
                if pipeline_name == "SegmentationPipeline":
                    self.frame_pipeline = SegmentationPipeline()
                elif pipeline_name == "CaptioningPipeline":
                    self.frame_pipeline = CaptioningPipeline()
                elif pipeline_name == "TypingPipeline":
                    self.frame_pipeline = TypingPipeline()
            else:
                logger.warning("Invalid choice. Using default pipeline.")
        except ValueError:
            logger.warning("Invalid input. Using default pipeline.")
    ...
    def _processing_loop(self):
        """
        Pull frames off the queue and hand each one to the active FramePipeline.
        """
        ...
        while True:
            try:
                if (self.frame_counter % self.processing_interval) == 0:
                    ...
                    self.frame_pipeline.set_debug_mode(self.debug_mode)

                    try:
                        scene_events, display_payload = self.frame_pipeline.process_frame(
                            frame, frame_id, timestamp,
                            roi_points=frame_data.get('roi_points'),
                            roi_controls=frame_data.get('roi_controls'),
                        )
                    except Exception as e:
                        logger.exception("❌ Error in frame pipeline '%s': %s", self.frame_pipeline.name, e)
                        scene_events, display_payload = [], None
                        ...
                        if self.music_enabled and self.musician is not None:
                            try:
                                music_frame = self.musician(
                                    ...
                                    state=self.frame_pipeline.get_state(),
                                )
                ...
                logger.exception("❌ Error in frame processing loop: %s", e)

    # --- shared musician plumbing (used by both paths above) -----------------

    def _broadcast_display_update(self):
        """
        Broadcast the active pipeline's visual payload (e.g. segmentation overlay).
        """
        try:
    ...
    def get_current_state(self):
        ...
        return {
            ...
            'active_pipeline': getattr(self.frame_pipeline, 'name', None),
    ...
    def get_synchronized_display(self, for_main_ui=True):
        """
        Get synchronized frame + active-pipeline display data for the UI.
        """
    ...
    def shutdown(self):
        ...
        self.frame_queue.put(None)  # Shutdown signal (video path)
        self.event_queue.put(None)  # Shutdown signal (generic event path)
...
class SegmentationPipeline():
    ...
    def __init__(self):
            ...
            if model_type == 'yolo':
                if not model_path:
                    model_path = os.path.join(
                        os.path.dirname(__file__), '..', 'Segmentation',
        ...
        try:
            self.detector = Detector('roi-events')
            logger.info("✅ Scene Event Detector (ROI-based) initialized successfully")
        except Exception as e:
    ...
    # --- colormap helpers (moved verbatim from Processor) ----------------------
    ...
    def process_frame(self, frame, roi_points=None, roi_controls=None):
        """
            ...
            if self.detector is not None:
                try:
                    scene_events = self.detector(
                        ...
                        roi={'corners': roi_points, 'controls': roi_controls},
                    )
                except Exception as event_err:
                    logger.error("❌ Error detecting scene events: %s", event_err)
                    logger.error("Traceback:\n%s", traceback.format_exc())
 
            return scene_events, display_payload

        except Exception as e:
            logger.exception("❌ Error in segmentation pipeline: %s", e)
            return [], None

class CaptioningPipeline():
    """
    A pipeline that generates captions for video frames.
    """
...
class TypingPipeline():
    """
    A pipeline that generates typing effects for video frames.
    """
